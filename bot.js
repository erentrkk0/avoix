...
.setThumbnail()
.addField(`Avonix | Teşekkürler`, `**Selamlar, Ben Eren Türk (Avonix'in Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Avonix | Prefix`, `**Avonix Botun Prefixi(ön eki) = \`a!\`\n\n Değiştirebilmek için \`a!prefix\` Yazabilirsiniz.**`)
.addField(`Avonix | Nasıl Kullanılır?`, `**Avonix botun tüm özelliklerinden yararlanabilmek için sadece \`a!yardım\` yazmanız yeterlidir.**`)
.addField(`Avonix | Linkler`, `**Sohbet Kanalına a!davet Yazmanız Yeterlidir**`)
.setFooter(`Avonix | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
client.on("message", msg => {
  ...
  var regex = new RegExp("^(http[s]?:\\/\\/(www\\.)?|ftp:\\/\\/(www\\.)?|www\\.){1}([0-9A-Za-z-\\.@:%_\+~#=]+)+((\\.[a-zA-Z]{2,3})+)(/(.)*)?(\\?(.)*)?(.gg)");
   ...
   msg.delete()
    ...
    .setDescription(`${msg.author} Reklam Yasak Bunu Bilmiyormusun! ${client.emojis.cache.get("✔️")}`)
...
const embed1 = new Discord.MessageEmbed()
        .setColor("BLUE")
        .setDescription(`${msg.author} Küfür Etmemelisin**!** ${client.emojis.get("✔️")}`)
...
if (!oldMsg.guild) return;
  ...
  if (i == 'aktif') {
      ...
      try {
        ...
        const embed1 = new Discord.MessageEmbed()
        .setColor("BLUE")
        .setDescription(`${oldMsg.author} Ben Zeki Bir Botum Mesajını Editlersende Küfür Edemezsin**!** ${client.emojis.get("✔️")}`)
...
client.on('message', async message => {
  ...
  if (message.content.includes(`<@${kullanıcı.id}>`)) {
    if (afkdkullanıcı) {
      message.channel.send(`:rocket: **| \`${message.author.tag}\` Adlı Kullanıcı Artık AFK Değil. Hoşgeldin ${message.author.tag}**`)
      db.delete(`afk_${message.author.id}`)
    }
    if (afkkullanıcı) return message.channel.send(`${message.author}\`${kullanıcı.tag}\` **ŞUAN Afk. Sebep:** \`${sebep}\``)
  }

  if (!message.content.includes(`<@${kullanıcı.id}>`)) {
    if (afkdkullanıcı) {
      message.channel.send(`:rocket: **| \`${message.author.tag}\` Adlı Kullanıcı Artık AFK Değil. Hoşgeldin** <@` + message.author.id +`>`)
      db.delete(`afk_${message.author.id}`)
    }
  }
});

...
if(dil === 'en') {
  let transkllanç = message.mentions.users.first()
  if(!transkllanç) return message.channel.send("Please Tag One.");
  let kllanç = message.author
  let para = args[1]
  if(transkllanç == kllanç) return message.channel.send(`You cannot send money to yourself.`)
  if(transkllanç.bot == true) return message.channel.send(`You cannot send money to bots.`)
  if(!transkllanç) return message.channel.send(`You must enter a user. Correct usage;\n\`${client.ekoayarlar.botunuzunprefixi}transfer @${client.user.tag} 50\``)
  if(!para) return message.channel.send(`You must enter a certain amount. Correct usage;\n\`${client.ekoayarlar.botunuzunprefixi}transfer @${client.user.tag} 50\``)
  const bakiye = await db.fetch(`bakiyecdare-${kllanç.id}`);
  const hesapdurumu = await db.fetch(`hesapdurumcodare-${kllanç.id}`);
  const hesapismi = await db.fetch(`hesapismiçodare-${kllanç.id}`);
  ...
  if(!hesapdurumu) {
    message.reply(`First you need to create an account. ${client.ekoayarlar.botunuzunprefixi}account-create <Account Name>`)
  } else {
    if(hesapdurumu) {
      if(!hesapismi) {
        message.reply(`First you need to create an account. ${client.ekoayarlar.botunuzunprefixi}account-create <Account Name>`)
        ...
        if(hesapdurumu) {
          if(hesapismi) {
            if(bakiye < para) return message.channel.send(`:warning: Your money is insufficient!`)
            if(!transhesapdurumu) return message.channel.send(`The user you want to transfer does not have an account.`)
            if(transhesapdurumu) {
                db.add(`bakiyecdare-${message.author.id}`, -para)
                db.add(`bakiyecdare-${transkllanç.id}`, para)
                transkllanç.send(`${message.author.tag} user named you \`${para} ${client.ekoayarlar.parabirimi}\` sent`)
                message.channel.send(`${transkllanç} successfully \`${para} ${client.ekoayarlar.parabirimi}\` you sent.`)
              }
          }
        }
      }
